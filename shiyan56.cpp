...
void drawpoint(int location,int limit,unsigned char r,unsigned char g,unsigned char b){
	if (location*3>limit) return;
	*(file_p+location*3)=b;
	*(file_p+location*3+1)=g;
	*(file_p+location*3+2)=r;
}
void drawpoint_easy(int x,int y,int width,int height){
	int limit=width*height+53;
	int location=getpoint(x,y,width);
...
void paint(double a,double b,double y1,double y2,int width,int height,double(*func)(double)){
	double step=1.0/STEP,start,x,y;
	...
	for (start=a;start<b;start+=step){
		...
		*(file_p_tmp+location*3)=0;
		*(file_p_tmp+location*3+1)=0;
		*(file_p_tmp+location*3+2)=0;
	}
}
void drawword(int x){
	int i,j;
...
int main(){
	...
	static long file_length=Init_bmp_head(width,height);
...
/*	for (i=0;i<file_length/3;i++)
	if ((i%width>=0&&i%width<=17)&&(i/width>=4&&i/width<=100))
	{
			*(file_p_tmp+i*3)=0xff;
			*(file_p_tmp+i*3+1)=0xff;
			*(file_p_tmp+i*3+2)=0xff;
}
*/
/*
	double a,b;
